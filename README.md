...
**84のシーンから毎回28問。** 日常の場面に加え、草むしり検定、討伐、仲間との食事など、ちいかわの世界を参考にした独自の質問が毎回10問登場します。場面ごとの行動を4択で選びます。原作の知識を試すクイズではありません。
...
性格モデル・テーマカラー・結果文章は `data.js`、84の場面と行動4択は `question-bank.js` にまとめています。
...
`node tools/browser-scenarios.mjs` では全84場面の320px幅での表示と、旧バージョンからの回答引き継ぎを確認します。

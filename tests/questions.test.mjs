...
test("140シーン・4択・測定枠の欠落なし", () => {
  assert.equal(questionBank.flat().length, 140);
  assert.equal(new Set(questionBank.flat().map((q) => q.text)).size, 140);
  assert.equal(questionBank.flat().filter((q) => q.story).length, 84);
...
test("毎回28問、作品シーン20問、直前と全問が異なる", () => {
  let previous = [];
  for (let seed = 0; seed < 100; seed++) {
    const next = createQuestionSet(seed, previous);
    assert.ok(validateQuestionSet(next));
    assert.equal(next.filter((q) => getQuestion(q).story).length, 20);
    for (const q of next)
      assert.ok(
